 ...
 *
 * StompFrames are handled in one of two ways.  If subscribe was called with
...
public abstract class Stomp {
    /**
     * A map of channel => listener pairs.  String => StompListener.
     */
    ...
    public abstract void connectedInit();
     ...
     */
    protected abstract void transmit( StompCommand command, Map header, String body );
     ...
     * @param header  A map of headers.  If null, an empty map is used.
     */
    protected void transmit( StompCommand command, Map header ) {
     ...
     * @param command The Stomp command.  If null, causes an error.
     */
    protected void transmit( StompCommand command ) {
        transmit( command, null, null );
    }


    /**
     * Begins a transaction.  StompFrames will not be delivered to 
     * subscribers until commit() has been called.
     */
    public void begin() { transmit( StompCommand.BEGIN ); }
     ...
     */
    public void begin( Map header ) { transmit( StompCommand.BEGIN, header ); }
     ...
     */
    public void commit() { transmit( StompCommand.COMMIT ); }
     ...
     * @param header Additional headers to send to the server.
     */
    public void commit( Map header ) { transmit( StompCommand.BEGIN, header ); }
    ...
    public void commitW( Map header ) throws InterruptedException { 
        ...
        transmit( StompCommand.COMMIT, header ); 
        waitOnReceipt( receipt );
    }


    /**
     * Aborts a transaction.  StompFrames sent since begin() was called
     * are destroyed and are not sent to subscribers.
     */
    public void abort() { transmit( StompCommand.ABORT ); }
     ...
     * @param header Additional headers to send to the server.
     */
    public void abort( Map header ) { transmit( StompCommand.ABORT, header ); }
     ...
     */
    public void subscribe( String name, StompListener listener ) {
     ...
     * @param listener A listener to receive messages sent to the channel
     */
    public void subscribe( String name,  StompListener listener, Map headers ) {
        ...
        headers.put("id", "andsub-8");

        System.out.println("55ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss");
    
        transmit( StompCommand.SUBSCRIBE, headers );
    }
     ...
     * @param listener A listener to receive messages sent to the channel
     */
    public void subscribeW( String name, StompListener listener, Map header ) 
     ...
     * @param listener A listener to receive messages sent to the channel
     */
    public void subscribeW( String name, StompListener listener ) 
        throws InterruptedException {
     ...
     */
    public void unsubscribe( String name, StompListener l ) {
        synchronized (_listeners) {
            List list = (List)_listeners.get( name );
            if (list != null) {
                list.remove( l );
    ...
    public void unsubscribe( String name, Map header ) {
        ...
        transmit( StompCommand.UNSUBSCRIBE, header );
    }
    ...
    public void send( String dest, String mesg, Map header ) {
        ...
        transmit( StompCommand.SEND, header, mesg );
    }
     ...
     */
    public StompFrame getNext() {
        synchronized(_queue) { return (StompFrame)_queue.pop(); }
    }
     ...
     */
    public StompFrame getNext( String name ) {
        synchronized( _queue ) {
            for (int idx = 0; idx < _queue.size(); idx++) {
                StompFrame m = (StompFrame)_queue.get(idx);
    ...
    public void addErrorStompListener( StompListener l ) {
        synchronized (_error_listeners) { _error_listeners.add( l ); }
    }

    public void delErrorStompListener( StompListener l ) {
            ...
            synchronized( _listeners ) {
                ...
                if (listeners != null) {
                    ...
                    for (Iterator i = listeners.iterator(); i.hasNext(); ) {
                        StompListener l = (StompListener)i.next();
                        try {
                            l.message( h, b );
                        } catch (Exception e) {
                    ...
                    _queue.push( new StompFrame( c, h, b ) );
                }
            }

        } else if (c == StompCommand.CONNECTED ) {
            System.out.println("-----------------------------+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++_------------------------------------------------------ stomp is connected");
            _connected = true;

            connectedInit();
        } else if (c == StompCommand.RECEIPT ) {
            _receipts.add( h.get("receipt-id") );
            synchronized(_receipts) { _receipts.notify(); }

        } else if (c == StompCommand.ERROR ) {
            if (_error_listeners.size() > 0) {
                synchronized (_error_listeners) {
                    for (Iterator i = _error_listeners.iterator(); i.hasNext(); ) {
                        try {
                            ((StompListener)i.next()).message( h, b );
                        } catch (Exception e) {
                            // Don't let listeners screw us over by throwing exceptions
                        }
                    }
    ...
    protected void reRegSubscribes()
    {
        Map<String, List<StompListener>> oliss = new HashMap<String, List<StompListener>>(_listeners);
        
        Iterator it = oliss.entrySet().iterator();
        while(it.hasNext()){
            Map.Entry pairs = (Map.Entry)it.next();

            String ckey = (String)pairs.getKey();
            List<StompListener> clis = (List<StompListener>)pairs.getValue();

            for(StompListener cl:clis){
                subscribe(ckey, cl);
            }
